...
for q in arange(2, 3, 1):
  ...
  for s1z in arange(-98, 1.0, .02):
   for s2z in arange(-.98, 1.0, .02):
      m1, m2 = mtotal_eta_to_mass1_mass2(M, eta)

      hp, hc = get_td_waveform(approximant='SEOBNRv3', mass1=m1, mass2=m2, spin1x=s1x, spin1y=s1y, spin1z=s1z, spin2x=s2x, spin2y=s2y, spin2z=s2z, f_lower=30., delta_t=1./8192.)
      ...
      for m1, m2, hp, hc in waveforms:
        ...
        pdata = hp.data[idx][0]
        cdata = hc.data[idx][0]
        savetxt('q_%.2f_s1z_%.2f_s2z_%.2f_f30.dat' % (m1/m2, s1z, s2z) ,\
