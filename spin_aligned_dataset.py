...
for q in arange(1, 2, 1):
  ...
  for s2z in arange(0, .9, .1)
    ...
    for m1, m2, hp, hc in waveforms:
      ...
      pdata = hp.data[idx][0]
      cdata = hc.data[idx][0]
      savetxt('q_%.2f_s1z_%.2f_s2z_%.2f_f15.dat' % (m1/m2, s1z, s2z) ,\
        zip(hp.sample_times.data / (M * 4.92549095e-6), hp.data / sqrt(pdata**2 + cdata**2), hc.data/ sqrt(pdata**2 + cdata**2)), fmt='%.18e\t%.18e\t%.18e')
